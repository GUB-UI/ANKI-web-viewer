...
| 指標 | 値 |
|---|---|
| 同一 Blob 3 回再生の `decodeAudioData` | 1 |
| 完了相当 `releaseAudioSession` 後の keep-alive | なし、`suspend` 1 回 |
| 遅延 resume 後のタイマー復活 | なし |
| 非表示中の unlock | keep-alive を開始しない |
| 欠損メディア 2 回目 | `media.where` を呼ばない |
| デッキ別 1 ヶ月ログ | 他デッキの当日ログを含まない |

実装後の本番ビルド:

| 成果物 | サイズ |
|---|---|
| 初期チャンク `index-*.js` | 539.53 kB（gzip 169.27 kB）。基準から約 82 kB / gzip 29 kB 減 |
| `ImportPage-*.js` | 54.10 kB（gzip 19.33 kB）。初回学習起動では読まない |
| `StatsPage-*.js` | 18.89 kB（gzip 6.14 kB）。初回学習起動では読まない |
| `index.html` が読むスクリプト | 初期チャンクのみ |
